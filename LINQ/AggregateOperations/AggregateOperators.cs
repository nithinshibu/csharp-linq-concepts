...
namespace LINQForBeginners.LINQ.AggregateOperations
{
    public class AggregateOperators
    {
        ...
        public void GroupedCount()
        {
            ...
            //Fetch the count of products available in each category
        ...
        public void MinGrouped()
        {
            List<Product> products = GetProductList();

            var categories = from p in products group p by p.Category into g select (Category: g.Key, CheapestPrice: g.Min(p => p.UnitPrice));
